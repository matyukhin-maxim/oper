...
class Journal extends Controller {
	...
	public function actionIndex() {
		...
		if ($this->open_shift_id > 0) {
			...
			// Сформируем список из доступных для редактирования журналов
			// и если они существуют, то будем рисовать галку рядом с сообщением,
			// для того, чтобы ползователь мог скопировать его в свой журнал
			$links = '';
			foreach ($this->authdata['journals'] as $journal) {
				if (get_param($journal, 'em') == 1)
					$links .= CHtml::createTag('li', null, CHtml::createLink(get_param($journal, 'description'), '#', [
						'class' => 'copy-messages strong',
						'data-journal' => get_param($journal, 'id'),
					]));
			}
			...
			$this->data['messageList'] = count($msgList) ? '' : CHtml::createTag('tr', ['class' => 'warning strong'], [
				CHtml::createTag('td', ['colspan' => 3], 'Сообщений нет'),
			]);
			foreach ($msgList as $message) {

				$special = get_param($message, 'special') ? 'special' : '';

				$this->data['messageList'] .= CHtml::createTag('tr', ['class' =>  $special], [
					CHtml::createTag('td', ['class' => 'text-center'], get_param($message, 'mdate')),
					CHtml::createTag('td', null, get_param($message, 'comment')),
					$links ? CHtml::createTag('td', null, CHtml::createTag('button', [
						'type' => 'button',
						'class' => 'btn btn-default btn-xs btn-check',
						'data-check' => 0,
						'data-id' => get_param($message, 'id'),
					], CHtml::createTag('i', ['class' => 'glyphicon'], '&nbsp;'))) : '',
				]);
			}

			if (!empty($links))
				$this->data['toolbar'] = CHtml::createTag('div', ['class' => 'dropup'], [
					CHtml::createButton('Копировать сообщения в', [
						'class' => 'btn btn-default btn-block btn-copy',
						'data-toggle' => 'dropdown',
						'aria-haspopup' => true,
					]),
					CHtml::createTag('ul', ['class' => 'dropdown-menu'], $links),
				]);

			if (!$editmode) $this->js[] = 'msg-copy'; else $this->data['toolbar'] = '';
	...
	public function actionPreview() {
		...
		$msgList = $this->model->getShiftMessages($sid);
		$this->data['messageList'] = count($msgList) ? '' : CHtml::createTag('tr', ['class' => 'warning strong'], [
			CHtml::createTag('td', ['colspan' => 3], 'Сообщений нет'),
		]);
		foreach ($msgList as $message) {
	...
	public function actionCopyMessages() {
		...
		$messages = filter_input(INPUT_POST, 'list', FILTER_VALIDATE_INT, ['flags' => FILTER_REQUIRE_ARRAY]);
		if (!$messages) {
			$this->appendDebug('Сообщения не выбраны', 3);
			$this->drawError();
			return;
		}

		$jid = filter_input(INPUT_POST, 'dest', FILTER_VALIDATE_INT);
		if (!$this->isRoleGranted('ACE_MESSAGE_ADD', $jid)) {
			$this->appendDebug('Нет прав на добавление сообщений в указанный журнал');
			$this->drawError();
			return;
		}

		// Проверим, открыта ли смена в жунале, куда ходим добавлять
		$shift_id = $this->model->getOpenShiftID($jid);
		if ($shift_id <= 0) {
			$this->appendDebug('Смена в выбранном журнале не открыта', 3);
			$this->drawError();
			return;
		}
		...
		foreach ($messages as $item) {
			...
			$source = $this->model->getMessageByID($item);
			$res &= $this->model->addNewMessage($shift_id,
				get_param($source, 'comment'),
				get_param($source, 'mdate'),
				get_param($this->authdata, 'id'))['ok'];
		}

		if ($res) $this->appendDebug('Сообщения скопированны!', 2);
		else $this->appendDebug('Скопировать сообщения не удалось. Обновите страницу, и попробуйте снова.');
