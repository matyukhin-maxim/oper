...
class JournalModel extends Model {
	...
	public function getJournalFullName($jid) {
		...
		$data = $this->select('SELECT description FROM journals WHERE id = :jid', ['jid' => $jid]);
		$result = get_param($data, 0);
		return get_param($result, 'description', 'n/a');
	}
	 ...
	 * @param bool $join Признак, чтовсе должности нужно соединить в один список
	 * @return array
	 */
	public function getAllUsersByPositions($ids = array(), $join = false) {
